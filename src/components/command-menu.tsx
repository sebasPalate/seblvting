...
import { SeblvtingMark, getMarkSVG } from "./seblvting-mark";
import { getWordmarkSVG } from "./seblvting-wordmark";
...
export function CommandMenu() {
  ...
  return (
    <>
      <button
        type="button"
        className="flex h-8 items-center gap-1.5 rounded-full border bg-zinc-50 px-3.5 text-muted-foreground select-none hover:bg-zinc-50 dark:bg-zinc-900 dark:hover:bg-zinc-900"
        ...
        <CommandList className="min-h-80 supports-timeline-scroll:scroll-fade-y">
          ...
          <CommandGroup heading="Brand Assets">
            <CommandItem
              onSelect={() => {
                handleCopyText(
                  getMarkSVG(resolvedTheme === "light" ? "#09090b" : "#ffffff"),
                  "Copied Mark as SVG",
                );
              }}
            >
  ...
  return (
    <>
      ...
      <div className="absolute inset-x-0 bottom-0 flex h-10 items-center justify-between gap-2 border-t bg-zinc-100/30 px-4 text-xs font-medium dark:bg-zinc-800/30">
        <SeblvtingMark className="size-6 text-muted-foreground" aria-hidden />
